...
module "web_instance" {
  ...
  iam_profile = module.S3_bucket.s3_iam_role.name
}
...
module "app_instance" {
  source = "./MODULES/EC2"
  aws_instance = var.app_aws_instance
  aws_instance_name = var.app_instance_name
...
module "rds" {
  ...
  parameter = var.P_parameter
  M_security_group_ids = [module.rds_sg.SECURITY_GROUP]
}
